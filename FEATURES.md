...
- **Installation** - VPS production guide ([`docs/deployment-vps.md`](docs/deployment-vps.md)): Python venv, MariaDB, Nginx, systemd, Certbot, UFW, fail2ban; legacy `bash install.sh` for quick dev setups
- **Maintenance** - Opt-in auto-update (`AUTO_UPDATE_ENABLED=False` by default), manual VPS updates, automated backups, log rotation, `/health/` endpoint

### VPS production deployment *(this fork)*
- **`deploy/clientst0r.service`** — gunicorn on `127.0.0.1:8000`, loads `/etc/clientst0r/.env`, runs as `clientst0r` user
- **`deploy/nginx-clientst0r.conf`** — Nginx reverse proxy to localhost gunicorn; static files; internal uploads alias
- **`deploy/clientst0r-scheduler.service`** + timer — background scheduled tasks
- **`deploy/logrotate-clientst0r`** — log rotation for `/var/log/itdocs` and `/var/log/clientst0r`
- **`deploy/clientst0r-fail2ban-*.conf`** — optional application jail
- **`.env.example`** — privacy-safe defaults; production secrets in `/etc/clientst0r/.env`
- **`manage.py check_safe_deployment`** — PASS/WARN/FAIL audit for production settings
- **Documentation** — [`docs/deployment-vps.md`](docs/deployment-vps.md), [`docs/security-hardening.md`](docs/security-hardening.md), [`AUTO_UPDATE.md`](AUTO_UPDATE.md)
